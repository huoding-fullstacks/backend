...
// 某日之前的
func GetDailyList(day string, count int) ([]*Daily, error) {
	dailys := []*Daily{}
	if err := connection.Default.Model(&Daily{}).Where("day < ?", day).Order("day desc").Limit(count).Find(&dailys).Error; err != nil {
